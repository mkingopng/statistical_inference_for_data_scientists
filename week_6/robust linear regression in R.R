...
# The coleman dataset (coleman.csv)
coleman <- read.csv(text="
...
# the least squares fit first
ls.m <- lm(formula=Y~salaryP+fatherWc+sstatus+teacherSc+motherLev,data=coleman)
summary(ls.m)
qqnorm(ls.m$residuals)

# the MM robust fit next
robfitMM <- rlm(formula=Y~salaryP+fatherWc+sstatus+teacherSc+motherLev,data=coleman,method="MM")
summary(robfitMM)

# fit using m estimator-not that different from MM fit (another robust fit)
robfitM <- rlm(formula=Y~salaryP+fatherWc+sstatus+teacherSc+motherLev,data=coleman,method="M")
summary(robfitM)

#effect of changing k
robfit1 <- rlm(formula=Y~salaryP+fatherWc+sstatus+teacherSc+motherLev,
data=coleman,psi=psi.huber,scale.est="Huber",k2=1.8,method="M")
summary(robfit1)